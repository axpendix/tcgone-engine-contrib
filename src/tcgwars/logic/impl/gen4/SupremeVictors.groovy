    ...
    switch (this) {
        ...
        return basic (this, hp:HP070, type:C, retreatCost:1) {
          ...
          move "Synchro Removal", {
            ...
            attackRequirement {
              def flag = false
              for(Type t1:Type.values()){
                if(self.cards.filterByType(ENERGY).find{it.asEnergyCard().energyTypes[0].contains(t1)} && defending.cards.filterByType(ENERGY).find{it.asEnergyCard().energyTypes[0].contains(t1)})
                  flag = true
              }
            ...
            onAttack {
              ...
              for(Type t1:Type.values()){
                if(self.cards.filterByType(ENERGY).find{it.asEnergyCard().energyTypes[0].contains(t1)} && defending.cards.filterByType(ENERGY).find{it.asEnergyCard().energyTypes[0].contains(t1)})
                  energyList.addAll(defending.cards.filterByType(ENERGY).findAll{it.asEnergyCard().energyTypes[0].contains(t1)})
              }
              if(energyList) {
      ...
      case NUMEL_74:
        return basic (this, hp:HP070, type:R, retreatCost:2) {
          ...
          move "Draw In", {
            ...
            onAttack {
              attachEnergyFrom(count:2,type:R,my.discard,self)
            }
          }
          move "Combustion", {
            text "40 damage. "
      ...
      case PLUSLE_76:
        return basic (this, hp:HP060, type:L, retreatCost:1) {
          ...
          move "Greedy Draw", {
            ...
            onAttack {
              draw 1 + opp.hand.size() - my.hand.size()
            }
          }
          move "Hand Charge", {
      ...
      case ROTOM_82:
        return basic (this, hp:HP070, type:L, retreatCost:1) {
          ...
          move "Discharge", {
            ...
            attackRequirement {}
            onAttack {
              flip self.cards.filterByEnergyType(L), {
                damage 40
              }
              afterDamage {
                self.cards.filterByEnergyType(L).discard()
              }
            }
          }
          move "Uproar", {
      ...
      case SKARMORY_FB_83:
        return basic (this, hp:HP080, type:M, retreatCost:1) {
          ...
          move "Metal Max", {
            ...
            onAttack {
              damage 20
              flip self.cards.filterByEnergyType(M), {
                damage 40
              }
              afterDamage {
                self.cards.filterByEnergyType(M).discard()
              }
            }
          }
        };
      ...
      case BELDUM_90:
        return basic (this, hp:HP050, type:M, retreatCost:2) {
          ...
          move "Pit Search", {
            ...
            onAttack {
              my.seck.search(test,cardTypeFilter(STADIUM)).showToOpponent("Pit Search: Selected card").moveTo(my.hand)
              shuffleDeck()
            }
          }
          move "Ram", {
          ...
          onPlay {
            eff = delayed {
              ...
              after LEVEL_UP, { if(flag) {
                ...
                heal(30, ef.pokemonToLevelUp, TRAINER_CARD)
              } }
            }
          }
          onRemoveFromPlay{
      ...
      case ABSOL_G_LV_X_141:
        return levelUp (this, from:"Absol G", hp:HP100, type:D, retreatCost:1) {
          ...
          pokePower "Darkness Send", {
            text "Once during your turn , when you put Absol LV.X from your hand onto your Active Absol , you may flip 3 coins. For each heads, put the top card from your opponent’s deck in the Lost Zone."
            onActivate {r->
          ...
          move "Darkness Slugger", {
            ...
            onAttack {
              damage 30
              if(my.hand && confirm("Discard a card to deal 30 additoinal damage?")) {
            ...
            delayedA {
              before APPLY_ATTACK_DAMAGES, {
                if(bg.em().retrieveObject("Burning Spirit") != bg.turnCount){
                  bg.dm().each {
                    if (it.from.active && it.to.isSPC(BURNED) && it.dmg.value) {//if not strong cheer has been used before
        ...
        return levelUp (this, from:"Electivire FB", hp:HP120, type:L, retreatCost:3) {
          ...
          pokePower "Energy Recycle", {
            ...
            actionA {
              ...
              powerUsed()
              attachEnergyFrom(max:3,my.discard,self)
              bg.gm().betweenTurns()
            }
          }
          move "Powerful Spark", {
      ...
      case GARCHOMP_C_LV_X_145:
        return levelUp (this, from:"Garchomp C", hp:HP110, type:C, retreatCost:0) {
          ...
          pokePower "Healing Breath", {
            ...
            onActivate {r->
              if (r==PLAY_FROM_HAND && my.all.find{it.numberOfDamageCounters} && confirm('Use Healing Breath?')) {
                powerUsed()
                my.all.each {
                  healAll(it)
                }
          ...
          move "Dragon Rush", {
            text "Discard 2 Energy attached to Garchomp C. Choose 1 of your opponent's Pokémon. This attack does 80 damage to that Pokémon. Garchomp C can’t use Dragon Rush during your next turn."
            energyCost C, C, C
            attackRequirement {}
            onAttack {
      ...
      case MOLTRES_149:
        return basic (this, hp:HP070, type:R, retreatCost:1) {
          resistance F, MINUS30
          move "Hyper Flame", {
            text "60 damage. Flip a coin. If heads, discard a [R] Energy card attached to Moltres. If tails, discard all [R] Energy cards attached to Moltres. If you can’t discard Energy cards, this attack does nothing."
            energyCost R, R, R
            attackRequirement {
      ...
      case ZAPDOS_150:
        return basic (this, hp:HP070, type:L, retreatCost:1) {
          weakness F
          move "Lightning Burn", {
            text "30 damage. Flip a coin. If heads, Zapdos does 30 damage to 1 of your opponent's Benched Pokémon. If tails, Zapdos does 30 damage to itself."
            energyCost L, L, L
            attackRequirement {}
            onAttack {
              damage 30
