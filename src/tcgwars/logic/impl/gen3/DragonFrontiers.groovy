    ...
    switch (this) {
        ...
        move "Dark Horn", {
          ...
          onAttack {
            ...
            if (my.hand.filterByType(POKEMON) && confirm("Discard Pokémon from Hand?")) {
              my.hand.filterByType(POKEMON).select("Which card to discard?").discard()
      ...
      return evolution (this, from:"Shellder", hp:HP070, type:F, retreatCost:1) {
        ...
        pokeBody "Solid Shell", {
          ...
          delayedA {
            before APPLY_ATTACK_DAMAGES, {
              bg.dm().each {
                if (it.from.owner!=self.owner && it.to.owner==self.owner && it.to.benched && it.dmg.value && it.to.topPokemonCard.cardTypes.is(DELTA)) {
            ...
            before null, null, ATTACK, {
              ...
              if (e.sourceAttack?.attacker?.owner == self.owner.opposite && ef.effectType != DAMAGE && pcs && pcs.owner == self.owner && pcs.benched && pcs.topPokemonCard.cardTypes.is(DELTA)) {
                bc "Solid Shell prevents effect (${ef.effectType})"
                prevent()
              }
            }
          }
      ...
      return evolution (this, from:"Seadra", hp:HP140, type:F, retreatCost:1) {
        ...
        pokeBody "Extra Smoke", {
          text "Any damage done to your Stage 2 Pokémon-ex by your opponent's attacks is reduced by 10."
          delayedA {
