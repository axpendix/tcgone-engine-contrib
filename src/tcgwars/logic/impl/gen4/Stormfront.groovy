      ...
      case TORTERRA_11:
        return evolution (this, from:"Grotle", hp:HP140, type:FIGHTING, retreatCost:4) {
          ...
          pokePower "Sunshine Song", {
            ...
            onActivate {r->
              if (r==PLAY_FROM_HAND && my.all.any{it.types.contains(G)} && my.deck && confirm("Use Sunshine Song?")) {
                powerUsed()
                multiSelect(my.all.findAll{it.types.contains(G)},0,99,"Select as many of you [G] Pokémon in play as you like.").each {pcs->
      ...
      case GENGAR_18:
        return evolution (this, from:"Haunter", hp:HP110, type:PSYCHIC, retreatCost:0) {
          ...
          pokePower "Fainting Spell", {
            text "Once during your opponent's turn, if Gengar would be Knocked Out by damage from an attack, you may flip a coin. If heads, the Attacking Pokémon is Knocked Out."
            ifActiveAndKnockedOutByAttack (delegate) { pcs->
              if (confirm("Use Fainting Spell?", self.owner)) {
                powerUsed()
                flip{
