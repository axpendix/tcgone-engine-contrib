...
//		bc "Shuffled $sel to deck"
  ...
  static void increasedDamageDoneToDefending(PokemonCardSet self, PokemonCardSet pcs, int value, String atkName = "") {
    targeted(pcs) {
      delayed {
        before APPLY_ATTACK_DAMAGES, {
          bg.dm().each {
            if (it.to == pcs && it.from.owner == self.owner && it.dmg.value > 0 && it.notNoEffect) {
  ...
  static void amnesia(def delegate) {
    ...
    afterDamage {
      if (defending.active && !defending.topPokemonCard.moves.isEmpty()) {
  ...
  static rearrangeEitherPlayersDeck(def delegate, int count) {
    ...
    delegate.onAttack {
      ...
      if (my().deck.notEmpty()) {
        choices.add(my().deck); labels.add("My deck")
      }
      if (opp().deck.notEmpty()) {
        choices.add(opp().deck); labels.add("Opp deck")
      }
