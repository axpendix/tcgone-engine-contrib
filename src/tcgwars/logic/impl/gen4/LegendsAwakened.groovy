        ...
        return basic (this, hp:HP100, type:C, retreatCost:3) {
          ...
          pokePower "Speed Gain", {
            ...
            actionA {
              ...
              flipUntilTails {
                def list = my.discard.filterByType(ENERGY).findAll {
                  it.containsTypePlain(R) || it.containsTypePlain(L)
                }
                if (list) {
                  attachEnergy(self, list.select("Attach which Energy to Rayquaza?").first())
                }
              }
            }
          }
          ...
          move "Tentacle Strike", {
            ...
            onAttack {
              if (opp.discard.filterByType(ENERGY)) {
                def energies = opp.discard.filterByType(ENERGY).select(min:1, max: 2, "Select up to 2 Energy Cards to attach")
          ...
          pokePower "JUNK", {
            ...
            onActivate { r->
              if (r == PLAY_FROM_HAND && bg.currentTurn == self.owner && my.discard.filterByType(ITEM) && confirm("Use $thisAbility?")) {
                powerUsed()
                flip {
                  my.discard.filterByType(ITEM).select(max:1, "Select a Trainer-Item card").moveTo(my.hand)
                }
              }
            }
          }
      ...
      case POKE_RADAR_133:
        return itemCard (this) {
          ...
          onPlay {
            ...
            top.select(min:0, max:top.filterByType(POKEMON).size(), "Choose the cards to put in your hand", cardTypeFilter(POKEMON)).moveTo(my.hand)
            shuffleDeck()
          }
          playRequirement{
            assert my.deck : "Deck is empty"
