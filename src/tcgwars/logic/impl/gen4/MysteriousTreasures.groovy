          ...
          pokeBody "Poison Sacs", {
            ...
            delayedA {
              before POISONED_SPC, null, null, CHANGE_STAGE, {
                if ((ef as Poisoned).getTarget().owner != self.owner && bg.em().currentEffectStack.find{it.effectType == EVOLVE || it.effectType == DEVOLVE || it.effectType == LEVEL_UP}) {
                  bc "$thisAbility prevents removing Poison"
                  prevent()
                }
              }
            }
      ...
      case CROCONAW_44:
        return evolution (this, from:"Totodile", hp:HP080, type:WATER, retreatCost:2) {
          ...
          pokePower "Evolutionary Vitality", {
            ...
            onActivate{reason ->
              if(reason == PLAY_FROM_HAND && my.deck.notEmpty && confirm("Use Evolutionary Vitality?")){
                powerUsed()
      ...
      case MAGMORTAR_LV_X_123:
        return levelUp (this, from:"Magmortar", hp:HP130, type:FIRE, retreatCost:3) {
          ...
          move "Flame Buster", {
            ...
            energyCost R, R, R, R
            attackRequirement {
              assert self.cards.filterByEnergyType(R).size() >= 2
            }
            onAttack {
              def cards = self.cards.filterByEnergyType(R).select(count: 2, "Discard")
