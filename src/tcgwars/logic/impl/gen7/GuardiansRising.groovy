		...
		switch (this) {
				...
				bwAbility "Roadblock", {
					...
					getterA (GET_BENCH_SIZE) {h->
						if(h.effect.playerType == self.owner.opposite) {
							h.object = Math.min(h.object as Integer, 4)
						}
					}
					onActivate {
			...
			return itemCard (this) {
				...
				onPlay {
					def i = 2
					while(i-- > 0){
						if ((bg.stadiumInfoStruct && bg.stadiumInfoStruct.stadiumCard.name != 'Heat Factory Prism Star' && bg.stadiumInfoStruct.stadiumCard.name != 'Life Forest Prism Star' && bg.stadiumInfoStruct.stadiumCard.name != 'Thunder Mountain Prism Star') && confirm("Would you like to discard stadium in play (${bg.stadiumInfoStruct.stadiumCard})? If not, you can select a Pokemon Tool in play")) {
				...
				playRequirement{
					assert all.findAll {it.cards.hasType(POKEMON_TOOL)} || (bg.stadiumInfoStruct && bg.stadiumInfoStruct.stadiumCard.name != 'Heat Factory Prism Star' && bg.stadiumInfoStruct.stadiumCard.name != 'Life Forest Prism Star' && bg.stadiumInfoStruct.stadiumCard.name != 'Thunder Mountain Prism Star')
				}
			};
			case HALA_126:
